...
func setup(game_data: Dictionary) -> void:
	...
	if time_start_label:
		time_start_label.text = game_data.get("time_start", "---")
	if position_label:
		position_label.text = game_data.get("position", "#?")
	if reward_amount_label:
		reward_amount_label.text = str(game_data.get("reward_amount", 0))
	if time_end_label:
		time_end_label.text = "FIN: " + game_data.get("time_end", "---")

	# 2. Configurar la lista de jugadores y visibilidad de puestos
	# Asumimos que game_data["player_names"] es un array de strings [ej: ["Lucas", "Crist", "July"]]
	var player_names = game_data.get("player_names", [])
